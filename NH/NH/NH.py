...
def play_game(secret, words, hard_mode=False):
    ...
    for attempt in range(1, attempts + 1):
        while True:
            ...
            if guess == "help":
                possible = [w for w in words if is_possible_for_help(w, guesses, feedbacks)]
                if possible:
...
#Help szo keresője
def is_possible_for_help(word, guesses, feedbacks):
    for guess, feedback in zip(guesses, feedbacks):
        required = [None] * 5         # Betűk, amiknek a helyükön kell lenniük
        present = []                  # Betűk, amik benne vannak, máshol
        excluded_positions = [set() for _ in range(5)]  # Betűk, amik nem lehetnek bizonyos pozíciókban
        excluded = set()             # Teljesen kizárt betűk
        min_counts = Counter()
        max_counts = {}
        ...
        for i in range(5):
            ...
            if f_char.isupper():
                ...
                present.append(g_char)
                excluded_positions[i].add(g_char)
                min_counts[g_char] += 1
                feedback_counts[g_char] += 1
            elif f_char == "_":
                feedback_counts[g_char] += 0
        ...
        for i in range(5):
            if required[i] and word[i] != required[i]:
                return False
            if word[i] in excluded_positions[i]:
                return False
...
def simple_ai_guess(secret, words):
    ...
    while True:
        candidates = [w for w in candidates if is_possible_for_help(w, guesses, feedbacks)]
        if not candidates:
